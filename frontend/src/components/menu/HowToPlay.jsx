...
const HowToPlay = () => {
    ...
    const steps = [
        ...
        },
        {
            ...
            desc: 'If the timer hits zero or you run out of attempts, the bomb explodes.'
        },
    ];

    const difficulties = [
        { level: 'EASY',   time: '90s', wires: 3, penalty: '-5s',  bonus: '+5s',  color: 'text-green-400'  },
        { level: 'NORMAL', time: '60s', wires: 5, penalty: '-10s', bonus: '+8s',  color: 'text-yellow-400' },
        { level: 'HARD',   time: '30s', wires: 7, penalty: '-15s', bonus: '+10s', color: 'text-red-400'    },
    ];
            ...
            <div className="max-w-md mx-auto">
                ...
                <div className="bg-gray-800 border border-green-700 rounded-lg p-4 mb-6">
                    ...
                    {difficulties.map((diff, index) => (
                        <div key={index} className="grid grid-cols-4 gap-2 text-sm bg-gray-700 rounded p-2 mb-2">
                            ...
                            <span className="text-gray-300">{diff.penalty} / {diff.bonus}</span>
                        </div>
                    ))}
                </div>
